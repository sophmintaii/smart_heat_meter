...
void GenericEventHandler(uint32 event, void * eventParam)
{
	...
	switch(event)
	{
		...
		case CYBLE_EVT_GAPP_ADVERTISEMENT_START_STOP:
            ...
            #ifdef TEMPERATURE_OUTPUT_ENABLED
            UART_UartPutString("*** Peripheral Advertisement START/STOP");
			UART_UartPutCRLF(' ');
            #endif
		...
		case CYBLE_EVT_GAPC_SCAN_START_STOP:
            ...
            #ifdef TEMPERATURE_OUTPUT_ENABLED
            UART_UartPutString("*** Central device is SCANNING");
			UART_UartPutCRLF(' ');
            #endif
		...
		case CYBLE_EVT_GATT_CONNECT_IND:
			...
			* is received for both Client or Server role */
            #ifdef TEMPERATURE_OUTPUT_ENABLED
            UART_UartPutString("*** Device is CONNECTED");
			UART_UartPutCRLF(' ');
            #endif
		...
		case CYBLE_EVT_GATT_DISCONNECT_IND:
			...
			* is received for both Client or Server role */
            #ifdef TEMPERATURE_OUTPUT_ENABLED
            UART_UartPutString("*** Device is DISCONNECTED");
			UART_UartPutCRLF(' ');
            #endif
		...
		case CYBLE_EVT_GATTS_WRITE_REQ:
			...
			* connected Client device */
            ...
            #ifdef TEMPERATURE_OUTPUT_ENABLED
            UART_UartPutString("*** WRITING TEMPERATURE...");
			UART_UartPutCRLF(' ');
            #endif
			...
			/* Extract temperature value and store it */
            TemperatureStorage.handleValPair.value.val[deviceIndex] = GetTemperature(flag_Timer);
            GREEN_LED_Pin_Write(0);
            
            #ifdef TEMPERATURE_OUTPUT_ENABLED
                SendTemperarureToUart(TemperatureStorage.handleValPair.value.val[deviceIndex]);
            #endif
            
			/* Update the RGB LED Control characteristic in GATT DB  to allow
			* Client to read the latest RGB LED color value set */
			CyBle_GattsWriteAttributeValue(&TemperatureStorage.handleValPair,0,&cyBle_connHandle,CYBLE_GATT_DB_LOCALLY_INITIATED);
            
            SendTempArrayToUart(TemperatureStorage.handleValPair.value.val);
			...
			/* After receiveing the temperature, set the switch role flag to allow the system
			* to switch role to Central role */
			switch_Role = TRUE;
            
            #ifdef TEMPERATURE_OUTPUT_ENABLED
            UART_UartPutString("*** Switch Role to CENTRAL");
			UART_UartPutCRLF(' ');
            #endif
		...
		case CYBLE_EVT_GATTS_WRITE_CMD_REQ:
			...
			* local variable. */
			...
			if(writeCmdData.handleValPair.attrHandle == CYBLE_CUSTOM_SERVICE_CUSTOM_CHARACTERISTIC_CHAR_HANDLE)
			{
			...
			* while scanning */
			if(CYBLE_STATE_CONNECTED != CyBle_GetState())	
			{
				...
				* information so to add it to our list */
                ...
                #ifdef TEMPERATURE_OUTPUT_ENABLED
                UART_UartPutString("*** Progress result of SCAN");
    			UART_UartPutCRLF(' ');
                #endif
					...
					* scan response data lenght...*/
					//if(scan_report.dataLen == SCAN_TAG_DATA_LEN) {
						...
						* then it is our desired node */
						...
						{
                            #ifdef TEMPERATURE_OUTPUT_ENABLED
                            UART_UartPutString("*** Device is FOUND. Stop SCANNING");
                			UART_UartPutCRLF(' ');
                            #endif
					...
					//} else {
                    //    UART_UartPutString("111111111111111f");
                    //}
				} 
			}
		break;
		...
		case CYBLE_EVT_GAP_DEVICE_CONNECTED:
			...
			if(ble_gap_state == BLE_CENTRAL)
			{
        		...
        		* RGB LED control data using attribute handles */
    			...
    			/* Extract temperature value and store it */
                TemperatureStorage.handleValPair.value.val[1] = GetTemperature(flag_Timer) - 0.12;
        		...
        		writeRGBdata.attrHandle = writeReqData.handleValPair.attrHandle;
        		writeRGBdata.value.val = TemperatureStorage.handleValPair.value.val;
        		writeRGBdata.value.len = 4;
        		CyBle_GattcWriteCharacteristicValue(cyBle_connHandle, &writeRGBdata);
                GREEN_LED_Pin_Write(0);
                
                #ifdef TEMPERATURE_OUTPUT_ENABLED
                UART_UartPutString("*** Data is WRITTEN");
    			UART_UartPutCRLF(' ');
