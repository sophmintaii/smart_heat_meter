...
uint8 UART_BT_initVar = 0u;
    ...
    volatile uint8 UART_BT_txBuffer[UART_BT_TXBUFFERSIZE];
    ...
    volatile uint8 UART_BT_rxBuffer[UART_BT_RXBUFFERSIZE];
    ...
    volatile uint8 UART_BT_rxBufferOverflow = 0u;
        ...
        volatile uint8 UART_BT_rxAddressMode = UART_BT_RXADDRESSMODE;
...
void UART_BT_Init(void) 
{
        ...
        #if(UART_BT_RX_INTERRUPT_ENABLED && (UART_BT_RXBUFFERSIZE > UART_BT_FIFO_LENGTH))
            /* Set the RX Interrupt. */
            (void)CyIntSetVector(UART_BT_RX_VECT_NUM, &UART_BT_RXISR);
            CyIntSetPriority(UART_BT_RX_VECT_NUM, UART_BT_RX_PRIOR_NUM);
        #endif /* End UART_BT_RX_INTERRUPT_ENABLED */

        #if (UART_BT_RXHW_ADDRESS_ENABLED)
            UART_BT_SetRxAddressMode(UART_BT_RXAddressMode);
            UART_BT_SetRxAddress1(UART_BT_RXHWADDRESS1);
            UART_BT_SetRxAddress2(UART_BT_RXHWADDRESS2);
        ...
        #if(UART_BT_TX_INTERRUPT_ENABLED && (UART_BT_TXBUFFERSIZE > UART_BT_FIFO_LENGTH))
        ...
        /* Write Counter Value for TX Bit Clk Generator*/
            ...
            UART_BT_TXBITCLKTX_COMPLETE_REG = (UART_BT_NUMBER_OF_DATA_BITS +
                        UART_BT_NUMBER_OF_START_BIT) * UART_BT_OVER_SAMPLE_COUNT;
        #else
...
void UART_BT_Enable(void) 
{
    ...
    enableInterrupts = CyEnterCriticalSection();
        ...
        #if(UART_BT_RX_INTERRUPT_ENABLED && (UART_BT_RXBUFFERSIZE > UART_BT_FIFO_LENGTH))
            CyIntEnable(UART_BT_RX_VECT_NUM);
            #if (UART_BT_RXHW_ADDRESS_ENABLED)
        ...
        #if(UART_BT_TX_INTERRUPT_ENABLED && (UART_BT_TXBUFFERSIZE > UART_BT_FIFO_LENGTH))
            CyIntEnable(UART_BT_TX_VECT_NUM);
        #endif /* End UART_BT_TX_INTERRUPT_ENABLED*/
     #endif /* End UART_BT_TX_ENABLED */

    #if(UART_BT_INTERNAL_CLOCK_USED)
        /* Enable the clock. */
        UART_BT_IntClock_Start();
    #endif /* End UART_BT_INTERNAL_CLOCK_USED */
...
void  UART_BT_WriteControlRegister(uint8 control) 
{
       ...
       UART_BT_CONTROL_REG = control;
        ...
        /*******************************************************************************
        * Function Name: UART_BT_EnableRxInt
        ********************************************************************************
        *
        * Summary:
        ...
        *******************************************************************************/
        ...
        /*******************************************************************************
        * Function Name: UART_BT_DisableRxInt
        ********************************************************************************
        *
        * Summary:
        ...
        *
        *******************************************************************************/
        void UART_BT_DisableRxInt(void) 
        {
            CyIntDisable(UART_BT_RX_VECT_NUM);
        }
    ...
    *  UART_BT_rxBufferLoopDetect - creared if loop condition was detected
    *     in RX ISR.
    *
    * Reentrant:
    *  No.
    ...
    *******************************************************************************/
    uint8 UART_BT_ReadRxData(void) 
    {
        uint8 rxData;
        ...
        #if(UART_BT_RXBUFFERSIZE > UART_BT_FIFO_LENGTH)
            ...
            /* Disable Rx interrupt. */
                ...
                UART_BT_DisableRxInt();
            #endif /* UART_BT_RX_INTERRUPT_ENABLED */
            loc_rxBufferRead = UART_BT_rxBufferRead;
            loc_rxBufferWrite = UART_BT_rxBufferWrite;

            if( (UART_BT_rxBufferLoopDetect != 0u) || (loc_rxBufferRead != loc_rxBufferWrite) )
            {
                rxData = UART_BT_rxBuffer[loc_rxBufferRead];
                loc_rxBufferRead++;

                if(loc_rxBufferRead >= UART_BT_RXBUFFERSIZE)
                {
                    loc_rxBufferRead = 0u;
                }
                /* Update the real pointer */
                UART_BT_rxBufferRead = loc_rxBufferRead;
        ...
        #else /* UART_BT_RXBUFFERSIZE > UART_BT_FIFO_LENGTH */

            /* Needs to check status for RX_STS_FIFO_NOTEMPTY bit*/
            rxData = UART_BT_RXDATA_REG;

        #endif /* UART_BT_RXBUFFERSIZE > UART_BT_FIFO_LENGTH */
    ...
    *******************************************************************************/
                                                            ...
                                                            
    {
        uint8 size;
            ...
            {
                size = (UART_BT_RXBUFFERSIZE - UART_BT_rxBufferRead) + UART_BT_rxBufferWrite;
            }
    ...
    *******************************************************************************/
    void UART_BT_ClearRxBuffer(void) 
    {
        ...
        enableInterrupts = CyEnterCriticalSection();
        UART_BT_RXDATA_AUX_CTL_REG |=  UART_BT_RX_FIFO_CLR;
    ...
    *******************************************************************************/
    void UART_BT_SetRxAddress2(uint8 address) 
    {
        ...
        /*******************************************************************************
        * Function Name: UART_BT_EnableTxInt
        ********************************************************************************
        *
        * Summary:
        ...
        *******************************************************************************/
        ...
        /*******************************************************************************
        * Function Name: UART_BT_DisableTxInt
        ********************************************************************************
        *
        * Summary:
        ...
        *
        *******************************************************************************/
        void UART_BT_DisableTxInt(void) 
        {
            CyIntDisable(UART_BT_TX_VECT_NUM);
        }
    ...
    *******************************************************************************/
    void UART_BT_WriteTxData(uint8 txDataByte) 
    {
        ...
        if(UART_BT_initVar != 0u)
        {
            #if(UART_BT_TXBUFFERSIZE > UART_BT_FIFO_LENGTH)
                ...
                {
                    if(UART_BT_txBufferWrite >= UART_BT_TXBUFFERSIZE)
                    {
            ...
            #else /* UART_BT_TXBUFFERSIZE > UART_BT_FIFO_LENGTH */

                /* Add directly to the FIFO. */
                UART_BT_TXDATA_REG = txDataByte;

            #endif /* End UART_BT_TXBUFFERSIZE > UART_BT_FIFO_LENGTH */
        }
    }
    ...
    void UART_BT_PutChar(uint8 txDataByte) 
    {
                ...
                */
                ...
                do{
                    ...
                    #if ((UART_BT_TXBUFFERSIZE > UART_BT_MAX_BYTE_VALUE) && (CY_PSOC3))
                        /* Disable TX interrupt to protect variables that could change on interrupt */
                        CyIntDisable(UART_BT_TX_VECT_NUM);
                    #endif /* End TXBUFFERSIZE > 255 */
                    loc_txBufferWrite = UART_BT_txBufferWrite;
                    loc_txBufferRead = UART_BT_txBufferRead;
                    #if ((UART_BT_TXBUFFERSIZE > UART_BT_MAX_BYTE_VALUE) && (CY_PSOC3))
                ...
                {
                    if(loc_txBufferWrite >= UART_BT_TXBUFFERSIZE)
                    {
                        loc_txBufferWrite = 0u;
                    }
                    /* Add to the software buffer. */
                    UART_BT_txBuffer[loc_txBufferWrite] = txDataByte;
                    loc_txBufferWrite++;
                    ...
                    /* Finally, update the real output pointer */
                        ...
                        CyIntDisable(UART_BT_TX_VECT_NUM);
                    #endif /* End TXBUFFERSIZE > 255 */
                    UART_BT_txBufferWrite = loc_txBufferWrite;
                    #if ((UART_BT_TXBUFFERSIZE > UART_BT_MAX_BYTE_VALUE) && (CY_PSOC3))
                        CyIntEnable(UART_BT_TX_VECT_NUM);
                    #endif /* End TXBUFFERSIZE > 255 */
                }
    ...
    *******************************************************************************/
    void UART_BT_PutString(const char8 string[]) 
    {
        ...
        if(UART_BT_initVar != 0u)
        {
            ...
            {
                UART_BT_PutChar((uint8)string[buf_index]);
                buf_index++;
            }
        }
    }
    ...
    *******************************************************************************/
                                                                    ...
                                                                    
    {
        ...
        if(UART_BT_initVar != 0u)
        {
            do
            {
                UART_BT_PutChar(string[buf_index]);
                buf_index++;
    ...
    *******************************************************************************/
                                                            ...
                                                            
    {
        uint8 size;
            ...
            {
                size = (UART_BT_TXBUFFERSIZE - UART_BT_txBufferRead) + UART_BT_txBufferWrite;
            }
    ...
    *******************************************************************************/
    void UART_BT_ClearTxBuffer(void) 
    {
        ...
        enableInterrupts = CyEnterCriticalSection();
        /* clear the HW FIFO */
        UART_BT_TXDATA_AUX_CTL_REG |=  UART_BT_TX_FIFO_CLR;
        ...
        if(UART_BT_initVar != 0u)
        {
            ...
            uint8 tmpStat;
                    ...
                    (retMode == UART_BT_SEND_WAIT_REINIT) )
                {
                    ...
                    #if(UART_BT_TXCLKGEN_DP)
                        tx_period = UART_BT_TXBITCLKTX_COMPLETE_REG;
                        UART_BT_TXBITCLKTX_COMPLETE_REG = UART_BT_TXBITCTR_BREAKBITS;
                    #else
                        tx_period = UART_BT_TXBITCTR_PERIOD_REG;
                    ...
                    (retMode == UART_BT_SEND_WAIT_REINIT) )
                {
                    ...
                    #if(UART_BT_TXCLKGEN_DP)
                        UART_BT_TXBITCLKTX_COMPLETE_REG = tx_period;
                    #else
                        UART_BT_TXBITCTR_PERIOD_REG = tx_period;
                    #endif /* End UART_BT_TXCLKGEN_DP */
    ...
    *******************************************************************************/
    void UART_BT_LoadTxConfig(void) 
    {
        ...
        UART_BT_RXBITCTR_PERIOD_REG = UART_BT_HD_TXBITCTR_INIT;
            ...
            CY_GET_REG8(UART_BT_RXSTATUS_PTR);
        #endif /* CY_UDB_V0 */
    }
